...
def track_position(scene,travel):
    ...
    px=p.x*1920+100*travel;py=(1-p.y)*1200+16.5*travel
...
def configure_camera(scene,camera):
    ...
    camera.location=(8,-40,30);aim(camera,(-.8,0,.667))
    ...
    x0,x1,y0,y1=bounds();camera.data.lens*=550/(x1-x0)
    bpy.context.view_layer.update();x0,x1,y0,y1=bounds()
    camera.data.shift_x=((x0+x1)/2-872)/1920
    camera.data.shift_y=(775-(y0+y1)/2)/1920
    bpy.context.view_layer.update()
    ...
    scene['assembly_camera_fit']='Inferred elevated camera, nominal 550px assembly width centered at (872,775); reference 1920x1200'
...
def configure_plate(scene,recipe):
    ...
    name=choose_plate(recipe['seed']);path=PLATE_DIR/name
    ...
    if scene.render.resolution_x!=1920:
        scale=n.new('CompositorNodeScale');scale.inputs['Type'].default_value='Relative';scale.inputs['X'].default_value=scene.render.resolution_x/1920;scale.inputs['Y'].default_value=scene.render.resolution_y/1200
