...
def hashes():
    ...
    return result
before=hashes()
studio.apply_settings(scene,dict(plastic_ink_wear=.28,resolution=1200,samples=8,flashlight_camera='REFERENCE',flashlight_capture='ALL'))
...
for obj in scene.objects:
    if obj.get('inspection_region')=='BODY':
        ...
        assert mat['inspection_body_print_version']==2 and 'FEDERAL' in mat['inspection_body_print_text']
...
folder=root/'examples/body-print-study/final'
infos=[]
...
checked=0
previous=root/'examples/reference-recovery/final'
for info in infos:
    assert info['appearance_version']==7
...
studio.arrange_view()
studio.save_blend(root/'examples/body-print-study/Readable body printing.blend')
...
studio.atomic_json(folder/'verification.json',dict(passed=True,body_print_version=2,packed=True,
...
print('READABLE_BODY_PRINT_VERIFIED',flush=True)
